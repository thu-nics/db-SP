    ...
    if sparse.dim() == 3:
        ...
        if ring_degree == 1:
            ...
            for idx, w in zip(row_idx_list, row_w_list):
                ...
                for g in range(ring_degree):
                    if row_group_counts[g] < group_size_h:
                        if g == original_block:
                            candidate_groups.append((g, row_group_sums[g] - reward * w))
                        else:
            ...
            for idx, w in zip(col_idx_list, col_w_list):
                ...
                for g in range(ring_degree):
                    if col_group_counts[g] < group_size_w:
                        if g == original_block:
                            candidate_groups.append((g, col_group_sums[g] - reward * w))
                        else:
            ...
            new_row_perm_idx = torch.cat([
                row_perm_idx_groups_sorted[(row_perm_idx_groups_sorted >= i * group_size) & 
                                          (row_perm_idx_groups_sorted < (i + 1) * group_size)]
            ...
            new_col_perm_idx = torch.cat([
                col_perm_idx_groups_sorted[(col_perm_idx_groups_sorted >= i * group_size) & 
                                          (col_perm_idx_groups_sorted < (i + 1) * group_size)]
                for i in range(num_groups)
            ]).reshape(ring_degree, -1)
            ...
            for i in range(new_row_perm_idx.shape[0]):
                new_row_deperm_idx[i][new_row_perm_idx[i]] = torch.arange(new_row_perm_idx.shape[1], 
                                                                         device=new_row_perm_idx.device)

            new_col_deperm_idx = torch.empty_like(new_col_perm_idx)
            for i in range(new_col_perm_idx.shape[0]):
                new_col_deperm_idx[i][new_col_perm_idx[i]] = torch.arange(new_col_perm_idx.shape[1], 
                                                                         device=new_col_perm_idx.device)

            if iftoken:
                sparse_final = sparse_reordered.index_select(1, row_perm_idx_groups_sorted.view(-1)).index_select(
                    2, col_perm_idx_groups_sorted.view(-1)).contiguous()
            else:
                sparse_final = sparse_reordered
        ...
        for block in range(sparse.shape[0]):
            sparse_final, head_perm_idx, new_row_perm_idx, new_col_perm_idx, transpose_matrix_q, transpose_matrix_k, head_deperm_idx, new_row_deperm_idx, new_col_deperm_idx = hybrid_permute_v4(
                sparse[block], ulysses_degree, ring_degree, reward, ifhead, iftoken)
...
def hybrid_imbalance_ratio(sparse: torch.Tensor, ulysses_degree: int = 2, ring_degree: int = 4):
    ...
    if ring_degree == 1:
        ...
        max_val = (iter_1 + iter_2).max()
        mean_val = sums.sum() / num_devices
        x = max_val.float() / mean_val.float()
        return x.item()
    elif ring_degree == 4:
        ...
        max_val = (iter_1 + iter_2 + iter_3 + iter_4).max()
        mean_val = sums.sum() / num_devices
        x = max_val.float() / mean_val.float()
        return x.item()
    elif ring_degree == 8:
        iter_1 = torch.stack([sums[:, i, i] for i in range(8)], dim=0).max(dim=0).values
        iter_2 = torch.stack([sums[:, i, (i+1)%8] for i in range(8)], dim=0).max(dim=0).values
        iter_3 = torch.stack([sums[:, i, (i+2)%8] for i in range(8)], dim=0).max(dim=0).values
        iter_4 = torch.stack([sums[:, i, (i+3)%8] for i in range(8)], dim=0).max(dim=0).values
        iter_5 = torch.stack([sums[:, i, (i+4)%8] for i in range(8)], dim=0).max(dim=0).values
        iter_6 = torch.stack([sums[:, i, (i+5)%8] for i in range(8)], dim=0).max(dim=0).values
        iter_7 = torch.stack([sums[:, i, (i+6)%8] for i in range(8)], dim=0).max(dim=0).values
        iter_8 = torch.stack([sums[:, i, (i+7)%8] for i in range(8)], dim=0).max(dim=0).values
        max_val = (iter_1 + iter_2 + iter_3 + iter_4 + iter_5 + iter_6 + iter_7 + iter_8).max()
        mean_val = sums.sum() / num_devices
        x = max_val.float() / mean_val.float()
        return x.item()
...
def parse_args():
    ...
    parser.add_argument("--seqlen", type=int, default=1184*64, help="sequence length (default: 1024)")
    parser.add_argument("--use_bwd", action="store_true", help="whether to test backward pass (default: False)")
    parser.add_argument("--sp_ulysses_degree", type=int, default=None, help="sp_ulysses_degree (default: world_size)")
    parser.add_argument("--ring_impl_type", type=str, default="basic", choices=["basic", "zigzag", "basic_flashinfer"],
                        help="ring implementation type (default: basic)")
    parser.add_argument("--causal", action="store_true", help="whether to use causal attention (default: False)")
    parser.add_argument("--attn_impl", type=str, default="torch",
                        choices=["torch", "fa", "fa3", "flashinfer", "sage_fp16", "sage_fp8", "sparse_sage",
                                 "sage_fp8_sm90", "sage_fp16_triton", "sage_auto", "paro", "sparge"],
                        help="attention implementation type (default: torch)")
    parser.add_argument("--sparse_sage_l1", type=float, default=0.07, help="l1 for sparse sage attention (default: 0.07)")
    parser.add_argument("--sparse_sage_pv_l1", type=float, default=0.08, help="pv_l1 for sparse sage attention (default: 0.08)")
...
def log(msg, a, rank0_only=False):
    ...
    if rank0_only:
        if rank == 0:
            print(f"[Rank#0] {msg}: max {a.abs().max().item()}, mean {a.abs().mean().item()}", flush=True)
        return
    ...
    for i in range(world_size):
        if i == rank:
            if rank == 0:
                print(f"{msg}:")
            print(f"[Rank#{rank}] max {a.abs().max().item()}, mean {a.abs().mean().item()}", flush=True)
        dist.barrier()
...
if __name__ == "__main__":
    ...
    print(f"original imbalance ratio: {hybrid_imbalance_ratio(sparse_org.transpose(1, 2), sp_ulysses_degree, sp_ring_degree)}, "
          f"after permute: {hybrid_imbalance_ratio(sparse.transpose(1, 2), sp_ulysses_degree, sp_ring_degree)}")
    ...
    local_q = EXTRACT_FUNC_DICT[ring_impl_type](
        q, rank, world_size=world_size, rd=sp_ring_degree, ud=sp_ulysses_degree).detach().clone()
    local_k = EXTRACT_FUNC_DICT[ring_impl_type](
        k, rank, world_size=world_size, rd=sp_ring_degree, ud=sp_ulysses_degree).detach().clone()
    local_v = EXTRACT_FUNC_DICT[ring_impl_type](
        v, rank, world_size=world_size, rd=sp_ring_degree, ud=sp_ulysses_degree).detach().clone()

    if sparse is not None:
        local_sparse = EXTRACT_FUNC_DICT[ring_impl_type](
            sparse, rank, world_size=world_size, rd=sp_ring_degree, ud=sp_ulysses_degree).detach().clone()
    else:
        local_sparse = None

    if sparse_org is not None:
        local_sparse_org = EXTRACT_FUNC_DICT[ring_impl_type](
            sparse_org, rank, world_size=world_size, rd=sp_ring_degree, ud=sp_ulysses_degree).detach().clone()
    else:
    ...
    if args.attn_impl == "sparse_sage":
        from spas_sage_attn.autotune import load_sparse_attention_state_dict, extract_sparse_attention_state_dict
    ...
    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True,
        ...
        for _ in range(3):
            _ = usp_attn(local_q, local_k, local_v, dropout_p=dropout_p, causal=causal, window_size=window_size,
                         softcap=0.0, alibi_slopes=alibi_slopes, deterministic=deterministic,
                         return_attn_probs=True, sparse=local_sparse_org)
            torch.cuda.synchronize()

        with record_function("usp_attn_forward_org"):
            local_out_ref = usp_attn(local_q, local_k, local_v, dropout_p=dropout_p, causal=causal,
                                     window_size=window_size, softcap=0.0, alibi_slopes=alibi_slopes,
                                     deterministic=deterministic, return_attn_probs=True, sparse=local_sparse_org)
            torch.cuda.synchronize()

        print("-----------")
        for _ in range(3):
            local_out = usp_attn(local_q, local_k, local_v, dropout_p=dropout_p, causal=causal,
                                 window_size=window_size, softcap=0.0, alibi_slopes=alibi_slopes,
                                 deterministic=deterministic, return_attn_probs=True, sparse=local_sparse,
                                 head_perm_idx=head_perm_idx, head_deperm_idx=head_deperm_idx,
        ...
        for _ in range(3):
            out_ref, _, _ = flash_attn_func(q, k, v, dropout_p=dropout_p, causal=causal,
    ...
    local_dout = EXTRACT_FUNC_DICT[ring_impl_type](
        dout, rank, world_size=world_size, rd=sp_ring_degree, ud=sp_ulysses_degree).detach().clone()
    ...
    if rank == 1:
        ...
        prof.export_chrome_trace(f"./profile_rank{rank}_ulysses{sp_ulysses_degree}ring{sp_ring_degree}_ablation_all.json")