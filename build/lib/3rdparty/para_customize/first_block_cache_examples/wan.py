...
class PARO_WanAttnProcessor2_0:
    def __init__(self):
    ...
    def __call__(
        ...
        hidden_states = torch.empty((1, 40, 75600, 128), device = 'cuda', dtype=torch.bfloat16)
...
class NEW_WanAttnProcessor2_0:
    ...
    def __call__(
        ...
        hidden_states = customize_spas_sage_attn_meansim_cuda(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False, simthreshd1=0.1, cdfthreshd=0.9, return_sparsity=False
        ) # hidden_states, head_density

        # org_hidden_states = F.scaled_dot_product_attention(
        #     query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        # )
        # import ipdb; ipdb.set_trace();
        ...
        hidden_states = attn.to_out[0](hidden_states)
        hidden_states = attn.to_out[1](hidden_states)
        return hidden_states # hidden_states, head_density
...
class NEW_WanTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            out_bias=True,
            processor=NEW_WanAttnProcessor2_0(),
        )
        ...
        self.attn2 = Attention(
            ...
            processor=OrgWanAttnProcessor2_0(),
        )
    ...
    def forward(
        ...
        attn_output = self.attn1(hidden_states=norm_hidden_states, rotary_emb=rotary_emb) # attn_output, head_density
...
class NEW_WanTransformer3DModel(ModelMixin, ConfigMixin, CacheMixin):
    ...
    def forward(
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            ...
            # import ipdb; ipdb.set_trace();
            for block in self.blocks:
                hidden_states = block(hidden_states, encoder_hidden_states, timestep_proj, rotary_emb) #hidden_states, head_density
                # all_head_density.append(head_density)
            # all_head_density = torch.stack(all_head_density, dim=0)  # shape: [num_blocks, 40, ...]
