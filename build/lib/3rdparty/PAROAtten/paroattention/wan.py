...
def permute_attn_out(attn_out, permute_plan, i_block):
    ...
    for i_head in range(N_head):
        ...
        attn_out_image_part[:,i_head,:,:] = attn_out_image_part[:,i_head,:,:].clone().reshape(*permuted_shape).permute(*permute_dims_head_inv_extend).reshape([BS,N_image_token,N_dim])
...
class PARO_WanAttnProcessor2_0:
    ...
    def __call__(
        ...
        # print(torch.cuda.memory_allocated() / 1024**2, "MB allocated before dot product attention")
...
class PARO_WanTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            out_bias=True,
            processor=PARO_WanAttnProcessor2_0(),
        )
