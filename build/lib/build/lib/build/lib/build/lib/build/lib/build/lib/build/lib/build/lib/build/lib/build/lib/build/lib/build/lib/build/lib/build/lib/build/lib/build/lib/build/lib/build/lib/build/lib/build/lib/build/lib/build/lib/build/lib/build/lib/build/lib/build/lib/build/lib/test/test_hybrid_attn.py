...
if __name__ == "__main__":
    ...
    # given data for sparge test
    # data=torch.load("/mnt/public/chensiqi/query_key_value_dict.pt", map_location='cpu', weights_only=True)
    # q=data['query'].to(device).transpose(1,2).contiguous()
    # k=data['key'].to(device).transpose(1,2).contiguous()
    # v=data['value'].to(device).transpose(1,2).contiguous()
    # batch_size, seqlen, nheads, d = q.shape
    # print(q.shape)
    # perm_idx = None
    # deperm_idx = None
    # sparse = None
    # _, _, _, perm_idx, deperm_idx=greedy_partition_and_rearrange_ulysses8(head_density)



    # setting1：random sparse mask with given sparse ratio 
    sparse_ratio = 1
    shape = (nheads, seqlen // 64,  seqlen // 64)
    sparse = (torch.rand(shape, device=device) < sparse_ratio)
    half = sparse.shape[-1] // 2
    sparse[:,:,:half] = False
    sparse[:,:,half:] = True
    # ulysses 8/4/2 cut the third dimension
    # ring 8 cut the second dimension

    #setting2: load sparse mask from saved file
    block = 15
    ...
    # sparse = sparse_data['sparse'][0, block, :, :, :].to(device) 
    ...
    # sparse = sparse_data['sparse'][0, block, :, :, :].to(device)  # [40, 1182, 1182]
    ...
    #setting3: chessboard pattern for ring test

    # ring_world_size = sp_ring_degree
    # sparse = torch.zeros((40, 1184, 1184), dtype=torch.bool, device=device)
    # block_h = 1184 // ring_world_size
    # block_w = 1184 // ring_world_size
    ...
    # setting4: hybrid
    sparse_org = sparse.unsqueeze(0).transpose(1, 2).to(device).contiguous()
    ...
    # sparse = torch.zeros_like(sparse, dtype=torch.bool).to(device)

    # # all be one
    # sparse = torch.ones_like(sparse, dtype=torch.bool)
    ...
    if rank == 1 :
        ...
        prof.export_chrome_trace(f"profile/wan/profile_rank{rank}_ulysses{sp_ulysses_degree}ring{sp_ring_degree}_block{block}_chess.json")  # 可选：导出火焰图
