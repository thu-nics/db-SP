...
if HAS_PARO_ATTENTION:
    import paroattention
...
def select_flash_attn_impl(
    impl_type: AttnType, stage: str = "fwd-bwd", attn_processor: torch.nn.Module = None, sparse : torch.Tensor = None
):
    ...
    elif impl_type == AttnType.PARO:
        if not HAS_PARO_ATTENTION:
            raise ImportError("PAROAttention is not available!")

        if stage == "fwd-only":
            def paro_wrapper(q, k, v, sparse, **kwargs):
                return paroattention.paroattn_qk_int8_pv_fp16_cuda(
                    q, k, v, 
                    sparse,
                    tensor_layout="HND",
                    return_lse=True,
    ...
    elif impl_type == AttnType.SPARGE:
        if not HAS_SPARSE_SAGE_ATTENTION:
            raise ImportError("SparseSageAttention is not available!")
        if stage == "fwd-only":
            return partial(
                customize_spas_sage_attn_meansim_cuda,
                is_causal=False,
                simthreshd1=0.1, 
                cdfthreshd=0.9, 
                pvthreshd=100000,
