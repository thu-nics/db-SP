    ...
    for step in range(comm.world_size):
        ...
        if not causal or step <= comm.rank:
            ...
            if attn_type == AttnType.SPARGE:
                ...
                block_lse = block_lse.transpose(-2, -1)
            if attn_type == AttnType.SPARSE_SAGE or attn_type == AttnType.SPARGE:
...
def ring_flash_attn_backward(
    ...
    for step in range(kv_comm.world_size):
        ...
        if step <= kv_comm.rank or not causal:
            ...
            fn = select_flash_attn_impl(attn_type, stage="bwd-only",sparse = sparse)
            fn(
                dout,
                q,
                k,
...
class RingFlashAttnFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        softmax_lse = None
        if attn_type == AttnType.SPARGE:
            out, head_density = ring_flash_attn_forward(
                group,
                q,
                k,
                v,
