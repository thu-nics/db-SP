...
def parallelize_transformer_paro(transformer, *args, **kwargs):
    transformer_cls_name = transformer.__class__.__name__
    if False:
        pass
    elif transformer_cls_name.startswith("Flux"):
    ...
    adapter_module = importlib.import_module(f".{adapter_name}", __package__)
    parallelize_transformer_fn = getattr(adapter_module, "parallelize_transformer_paro")
    return parallelize_transformer_fn(transformer, *args, **kwargs)


def parallelize_pipe_paro(pipe: DiffusionPipeline, *args, **kwargs):
    assert isinstance(pipe, DiffusionPipeline)
    ...
    adapter_module = importlib.import_module(f".{adapter_name}", __package__)
    parallelize_pipe_fn = getattr(adapter_module, "parallelize_pipe_paro")
    return parallelize_pipe_fn(pipe, *args, **kwargs)

def parallelize_transformer_sparge(transformer, *args, **kwargs):
    transformer_cls_name = transformer.__class__.__name__
    if False:
        pass
    elif transformer_cls_name.startswith("Flux"):
    ...
    adapter_module = importlib.import_module(f".{adapter_name}", __package__)
    parallelize_transformer_fn = getattr(adapter_module, "parallelize_transformer_sparge")
    return parallelize_transformer_fn(transformer, *args, **kwargs)


def parallelize_pipe_sparge(pipe: DiffusionPipeline, *args, **kwargs):
    assert isinstance(pipe, DiffusionPipeline)
    ...
    adapter_module = importlib.import_module(f".{adapter_name}", __package__)
    parallelize_pipe_fn = getattr(adapter_module, "parallelize_pipe_sparge")
    return parallelize_pipe_fn(pipe, *args, **kwargs)
