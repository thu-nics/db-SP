...
def imbalance_ratio(sparse:torch.Tensor,num_devices:int=8):
    num_heads = sparse.shape[-1]
    assert num_heads % num_devices == 0
    group_sums = sparse.view(42, num_devices, -1).sum(dim=-1)
...
class CogVideoXPipeline_NEW(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if head_density is not None:
                    if imbalance_ratio(head_density) > 1.1:
                ...
                if head_density is not None:
                    ...
                    if dist.get_rank() == 0:
                        print(f"timestep:{i}, count:{count}, imbalance ratio: {imbalance_ratio(head_density)}, Transformer attention time: {total_time} ms")
