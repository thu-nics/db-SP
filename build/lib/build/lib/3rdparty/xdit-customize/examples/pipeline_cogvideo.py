...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ```
"""
def greedy_partition_and_rearrange_ulysses8_multi(sparse: torch.Tensor, old_perm_idx: list, old_deperm_idx:list, num_groups: int = 8):
    """
...
class NEW_CogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if head_density is not None:
                    ...
                    if imbalance_ratio(head_density, num_groups=dist.get_world_size()) > threshold and use_db_sp:
                        _, _, _, perm_idx, deperm_idx = greedy_partition_and_rearrange_ulysses8_multi(head_density, perm_idx, deperm_idx, num_groups=dist.get_world_size())
                        count += 1
                ...
                noise_pred, head_density = self.transformer(
                    ...
                    perm_idx=perm_idx if perm_idx is not None else None,
                    deperm_idx=deperm_idx if deperm_idx is not None else None,
                    return_dict=False,
                )
                noise_pred = noise_pred.float()

                if hasattr(self.transformer, "get_time_stats"):
                    time = self.transformer.get_time_stats()
                    total_time += time
                if head_density is not None:
                    import torch.distributed as dist
                    if dist.get_rank() == 0:
                        ...
                        print(f"timestep:{i}, count:{count}, imbalance: {imbalance_ratio(head_density, num_groups=dist.get_world_size())}, Transformer attention time: {time} ms")
        ...
        # plt.grid(True)
        # plt.savefig('/mnt/public/chensiqi/xDiT/results/timestep_cogvideo3.png')
        # plt.close()
        ...
        if not return_dict:
            return (video, total_time)

        output = CogVideoXPipelineOutput(frames=video)
        output.attention_time_ms = total_time
        return output 

class PARO_CogVideoXPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    Pipeline for text-to-video generation using CogVideoX.
